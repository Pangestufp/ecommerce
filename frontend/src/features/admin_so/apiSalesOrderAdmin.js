...
const ApiSalesOrderAdmin = {
  // Halaman pertama — direction next, tanpa cursor
  getFirstPage: async (statuses = []) => {
    const url = Endpoints.SALESORDER_ADMIN.GET_ALL_PAGINATE("", "", "next", statuses);
  ...
  getNextPage: async (paginate, statuses = []) => {
    const url = Endpoints.SALESORDER_ADMIN.GET_ALL_PAGINATE(
      paginate.last_id,
  ...
  getPrevPage: async (paginate, statuses = []) => {
    const url = Endpoints.SALESORDER_ADMIN.GET_ALL_PAGINATE_PREV(
      paginate.first_id,
  ...
  getByCode: async (code) => {
    const res = await api.get(Endpoints.SALESORDER_ADMIN.GET_BY_CODE(code));
    return res;
  },
};

export default ApiSalesOrderAdmin;